          ...
          <div className="relative container mx-auto px-4 sm:px-6 py-20 sm:py-32 text-center text-white">
            <div className="max-w-4xl mx-auto space-y-8">
              ...
              <p className="text-lg sm:text-2xl lg:text-3xl font-medium opacity-95 drop-shadow-lg max-w-3xl mx-auto">
                Discover instant fun with jokes, games, amazing facts, and endless surprises!
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center pt-4">
                <Link href="/p/random-activity">
                  <button className="group px-8 sm:px-12 py-4 sm:py-6 text-xl sm:text-2xl font-bold bg-white text-purple-600 rounded-full shadow-2xl hover:shadow-purple-400/50 hover:scale-110 transition-all duration-300 transform hover:-rotate-2">
                    🎉 Kill My Boredom Now!
                    <span className="inline-block group-hover:translate-x-2 transition-transform ml-2">→</span>
                  </button>
                </Link>
              </div>
        ...
        <section className="py-12 sm:py-16 bg-white/60 backdrop-blur-sm">
          <div className="container mx-auto px-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-12 max-w-5xl mx-auto">
              {[
                { emoji: '😴', title: 'Feel Bored', desc: 'Staring at the wall? Scrolling endlessly? We get it.' },
                { emoji: '👆', title: 'Click Anything', desc: 'Pick a category or hit the big button. It\'s that simple!' },
                { emoji: '😄', title: 'Have Fun Instantly', desc: 'Laugh, play, learn — boredom vanishes in seconds!' }
              ].map((step, idx) => (
                <div key={idx} className="group relative bg-white p-6 sm:p-8 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border border-purple-100">
                  ...
                  <div className="relative text-center space-y-4">
                    ...
                    <h3 className="text-xl sm:text-2xl font-bold text-gray-800">{step.title}</h3>
            ...
            <div className="lg:col-span-3 space-y-12 sm:space-y-16">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                  {[
                    { title: 'Random Jokes', desc: 'Instant laughs guaranteed', emoji: '😂', color: 'from-blue-500 to-blue-600', href: '/jokes' },
                    { title: 'Fun Facts', desc: 'Mind-blowing trivia daily', emoji: '🧠', color: 'from-green-500 to-green-600', href: '/facts' },
                    { title: 'Quick Games', desc: 'Play in your browser', emoji: '🎮', color: 'from-orange-500 to-orange-600', href: '/games' },
                    { title: 'DIY Ideas', desc: 'Creative projects at home', emoji: '✂️', color: 'from-purple-500 to-purple-600', href: '/diy' },
                    { title: 'Weird Web', desc: 'Hilarious & strange sites', emoji: '🌐', color: 'from-pink-500 to-pink-600', href: '/weird' },
                    { title: 'Relax & Chill', desc: 'Calming vibes', emoji: '😌', color: 'from-yellow-500 to-yellow-600', href: '/relax' },
                  ].map((cat) => (
                    <Link key={cat.title} href={cat.href}>
                      <div className={`group relative p-6 sm:p-8 rounded-2xl bg-gradient-to-br ${cat.color} text-white shadow-xl hover:shadow-2xl hover:-translate-y-3 transition-all duration-300 cursor-pointer overflow-hidden`}>
                        ...
                        <div className="relative text-center space-y-3">
                          ...
                          <p className="text-base sm:text-lg opacity-90">{cat.desc}</p>
                        </div>
                      </div>
                    </Link>
                  ))}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {[
                    { title: 'Dad Joke of the Day', emoji: '🎭', color: 'from-purple-400 to-purple-500' },
                    { title: 'Mind-Bending Riddle', emoji: '🧩', color: 'from-pink-400 to-pink-500' },
                    { title: '5-Minute Browser Game', emoji: '🎮', color: 'from-blue-400 to-blue-500' },
                    { title: 'Crazy Animal Fact', emoji: '🦁', color: 'from-green-400 to-green-500' },
                    { title: 'Quick DIY Craft', emoji: '✨', color: 'from-orange-400 to-orange-500' },
                    { title: 'Relaxing Soundscape', emoji: '🎵', color: 'from-indigo-400 to-indigo-500' }
                  ].map((item, i) => (
                    <Link key={i} href="/random">
                      <div className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl overflow-hidden transition-all duration-300 hover:-translate-y-2 cursor-pointer border border-purple-100">
                        <div className={`bg-gradient-to-br ${item.color} h-40 sm:h-48 flex items-center justify-center text-6xl sm:text-7xl transform group-hover:scale-110 transition-transform duration-300`}>
                          {item.emoji}
                        </div>
                        <div className="p-5 sm:p-6">
                          <h3 className="text-lg sm:text-xl font-bold mb-2 text-gray-800">{item.title}</h3>
                          <p className="text-gray-600 mb-4 text-sm sm:text-base">Trending today! Click to try it.</p>
              ...
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-8 sm:p-12 rounded-3xl shadow-xl border border-purple-100">
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  {[
                    { icon: '🎯', title: 'Always Fresh', desc: 'New content added daily to keep things exciting' },
                    { icon: '🚀', title: 'Super Fast', desc: 'No waiting, no loading. Instant entertainment' },
                    { icon: '👨‍👩‍👧‍👦', title: 'Family Friendly', desc: 'Safe, clean fun for all ages' },
                    { icon: '📱', title: 'Works Everywhere', desc: 'Phone, tablet, computer—we got you covered' }
                  ].map((item, i) => (
                    <div key={i} className="flex gap-4 items-start bg-white p-5 rounded-xl shadow-md hover:shadow-lg transition-shadow">
            ...
            <aside className="space-y-8">
              ...
              <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-xl border border-purple-100">
                ...
                <div className="space-y-6">
                  {[
                    { num: '50K+', label: 'Jokes Told', color: 'text-purple-600' },
                    { num: '120K+', label: 'Happy Visitors', color: 'text-pink-600' },
                    { num: '1M+', label: 'Boredom Killed', color: 'text-green-600' }
                  ].map((stat, i) => (
                    <div key={i} className="text-center p-4 bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl">
                      <div className={`text-3xl sm:text-4xl font-black ${stat.color} mb-1`}>{stat.num}</div>
              ...
              <div className="bg-gradient-to-br from-purple-100 to-pink-100 p-6 rounded-2xl border border-purple-200">
                <h3 className="text-lg font-bold mb-4 text-gray-800">⚡ Quick Jump</h3>
                <div className="space-y-3">
                  {['Surprise Me!', 'Daily Challenge', 'Top Rated', 'Random Pick'].map((link) => (
        ...
        <section className="py-12 sm:py-16 bg-gradient-to-br from-purple-100 via-pink-100 to-orange-100">
          <div className="container mx-auto px-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 lg:gap-12 max-w-6xl mx-auto">
              {[
                { quote: 'This site saved my lunch break every single day! So much fun and variety!', name: 'Sarah K.', rating: 5 },
                { quote: 'Best random jokes on the internet! I laugh out loud every time.', name: 'Mike T.', rating: 5 },
                { quote: 'My kids absolutely love the games section. Clean, safe, and entertaining!', name: 'Emma L.', rating: 5 }
              ].map((t, i) => (
                <div key={i} className="bg-white p-6 sm:p-8 rounded-2xl shadow-xl hover:shadow-2xl transition-shadow border border-purple-100">
                  <div className="flex justify-center mb-4 text-yellow-400 text-xl">
                    {'⭐'.repeat(t.rating)}
                  </div>
                  <p className="text-base sm:text-lg italic mb-6 text-gray-700 leading-relaxed">&quot;{t.quote}&quot;</p>
                  <p className="font-bold text-purple-600 text-center">— {t.name}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-12 sm:py-16 bg-gradient-to-r from-purple-600 via-pink-500 to-orange-500 text-white relative overflow-hidden">
          ...
          <div className="relative container mx-auto px-4 sm:px-6 text-center">
            ...
            <p className="text-lg sm:text-xl lg:text-2xl mb-6 sm:mb-8 opacity-95 max-w-2xl mx-auto">
              Get daily fun, jokes, and surprises delivered straight to your inbox!
            </p>
            <div className="max-w-md mx-auto flex flex-col sm:flex-row gap-3 sm:gap-4">
              ...
              <p className="mt-4 text-sm opacity-75">✅ Free forever • 📬 No spam • 🔒 Unsubscribe anytime</p>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4 sm:px-6">
            <div className="text-center space-y-6">
              ...
              <p className="text-gray-400 text-sm sm:text-base max-w-2xl mx-auto">
                Made with ❤️ to cure boredom worldwide. Join thousands who&apos;ve already escaped the monotony!
              </p>
              <div className="flex flex-wrap justify-center gap-4 sm:gap-6 text-sm sm:text-base">
                {['About', 'Privacy Policy', 'Terms', 'Contact', 'Sitemap'].map((link) => (
                  <Link key={link} href={`/${link.toLowerCase().replace(' ', '-')}`}>
                    <span className="hover:text-purple-400 transition-colors cursor-pointer">{link}</span>
                  </Link>
                ))}
              </div>
