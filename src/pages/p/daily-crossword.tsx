...
export default function DailyCrossword() {
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] text-slate-900 font-sans pb-12">
      <Head>
        <title>The Master Mini | Word Lab</title>
      </Head>
      ...
      <nav className="bg-white/70 backdrop-blur-xl border-b border-white py-4 px-8 sticky top-0 z-50 flex justify-between items-center shadow-sm">
        <div className="flex flex-col">
          <h1 className="text-2xl font-black tracking-tighter italic uppercase text-indigo-900 leading-none">
            The Master
          </h1>
        ...
        <div className="grid lg:grid-cols-[1fr_400px] gap-8 items-start">
              ...
              >
                {gridState.map((row, r) =>
                  row.map((cellVal, c) => {
                    ...
                    const isCorrect =
                      cellVal !== "" && cellVal === DAILY_PUZZLE.grid[r][c];
                    const isWrong =
                      cellVal !== "" && cellVal !== DAILY_PUZZLE.grid[r][c];

                    if (isBlock)
                      return <div key={`${r}-${c}`} className="bg-slate-900" />;
                    return (
                      <div
                        key={`${r}-${c}`}
                      ...
                      >
                        {getCellNumber(r, c) && (
                          <span className="absolute top-0.5 left-0.5 text-[8px] md:text-[10px] font-black text-slate-400">
                            {getCellNumber(r, c)}
                          </span>
                        )}
                        <input
          ...
          <div className="space-y-6">
            <div className="bg-indigo-900 text-white p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group">
              ...
              <div className="absolute -bottom-4 -right-4 text-8xl opacity-10 font-black italic group-hover:scale-110 transition-transform">
                {direction === "across" ? "→" : "↓"}
              </div>
            </div>
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 bg-white border border-slate-200 py-4 rounded-3xl text-xs font-black uppercase hover:shadow-lg transition-all active:scale-95"
              >
                ↻ Switch
              </button>
              <button
                onClick={revealHint}
                className="flex-1 bg-white border border-slate-200 py-4 rounded-3xl text-xs font-black uppercase hover:shadow-lg transition-all active:scale-95 text-purple-600"
              >
                💡 Hint
              </button>
            </div>
                  ...
                  <div key={dir}>
                    ...
                    <div className="space-y-3">
                      {(DAILY_PUZZLE.clues as any)[dir].map((c: any) => (
                        <div
                          key={c.number}
                          className={`text-sm flex gap-3 ${direction === dir && getCurrentClue() === c.text ? "text-indigo-600 font-bold" : "text-slate-500"}`}
                        >
      ...
      {isWon && (
        <div className="fixed inset-0 bg-indigo-950/90 backdrop-blur-md z-[100] flex items-center justify-center p-6 animate-in fade-in duration-500">
          <div className="bg-white rounded-[3rem] p-12 text-center max-w-sm w-full shadow-[0_0_50px_rgba(79,70,229,0.3)]">
            ...
            <h2 className="text-4xl font-black italic tracking-tighter uppercase text-slate-900 mb-2">
              Decathlon Champion!
            </h2>
            ...
            <button
              ...
              className="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black uppercase shadow-xl shadow-indigo-200 hover:scale-105 transition-transform"
            >
              Next Challenge
            </button>
          </div>
